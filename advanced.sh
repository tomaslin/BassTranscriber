#!/usr/bin/env bash
# ==============================================================================
# Bass Transcription Pipeline (M1 / Apple Silicon Optimized Edition v7.0)
...
# ==============================================================================
# EXPANDED SUBGENRE OPTIMIZATION REGISTRY (NOW INCLUDING ALL 23 PROFILES)
# ==============================================================================
SUBGENRE_REGISTRY = {
...
def auto_detect_profile(bass_wav_path, drums_wav_path):
    ...
    # 1. Bass Features: Brightness (Spectral Centroid) & ZCR
    cent = librosa.feature.spectral_centroid(y=y_b, sr=sr_b)
    avg_centroid = np.median(cent)
    
    zcr = librosa.feature.zero_crossing_rate(y=y_b)
    avg_zcr = np.median(zcr)

    # 2. Rhythm Features: Utilize Drums Stem if available for precise density and BPM
    ...
    # 3. Statistical Distribution Matching (Z-Score / Normalized Euclidean)
    # Modeled after Music Information Retrieval (MIR) datasets like GTZAN.
    # Format: "genre": {"bpm": (mean, std), "centroid": (mean, std), "density": (mean, std), "zcr": (mean, std)}
    academic_profiles = {
    ...
    # Optional: allow half-time/double-time BPM matching
    ...
    for g, stats in academic_profiles.items():
        for b_variant in bpm_variants:
            ...
            # Calculate Z-score squared for each feature
            ...
            # Weighted sum of Z-scores (can adjust weights based on feature reliability)
    ...
    print(f"  -> Analyzed BPM: {bpm:.1f}")
    print(f"  -> Analyzed Tone: {avg_centroid:.0f}Hz")
    print(f"  -> Analyzed ZCR: {avg_zcr:.3f}")
    print(f"  -> Analyzed Density: {note_density:.1f} attacks/sec")
    print(f"  -> Profile Assigned: [{best_genre.upper()}]")
...
def estimate_harmonic_key(y, sr):
    ...
    # Krumhansl-Schmuckler Key Profiles (MIR Standard)
    ...
    sharp_keys_maj = ['C', 'G', 'D', 'A', 'E', 'B', 'F#', 'C#']
    flat_keys_maj = ['F', 'Bb', 'Eb', 'Ab', 'Db', 'Gb', 'Cb']
        ...
        if i == 0:
            ...
            for vs in valid_states:
                ...
                for ps, (prev_cost, prev_hist) in prev_paths.items():
                    # Biomechanical Hand-Span Cost Model (Academic Approach)
                    # Hand span is typically 4 frets. Cost increases exponentially outside this span.
                    if vs[1] == 0:
                        fret_cost = 0.5 # Open strings are easy to transition to
                    elif ps[1] == 0:
                        # Coming from an open string, penalize higher frets to anchor hand position
                        fret_cost = 1.0 + (vs[1] * 0.5)
                    else:
                        span = abs(vs[1] - ps[1])
                        if span <= 4:
                            fret_cost = span * 0.5 # Low linear cost within hand span
                        else:
                            fret_cost = 2.0 + ((span - 4) ** 2) # Exponential penalty for shifting
                            
                    # Additional heuristic: thick strings played high up sound muddy and are less ergonomic
                    if vs[1] > 7 and vs[0] < 2:
                        fret_cost += (vs[1] - 7) * 1.5
                            
                    string_cost = abs(vs[0] - ps[0]) * 1.5 # Cost for string skipping
...
def extract_timbre(y_b, sr_b, start_time, end_time):
    ...
    # 2. Spectral Flux (Onset transient energy) - Robust MIR feature for percussive slaps/pops
    # Slap and pop techniques have high high-frequency transient energy.
    ...
    # 3. Spectral Rolloff (indicates high-frequency content)
...
def process_score_tier(raw_notes, pitch_bends, level, genre, bpm, detected_key, tuning, y_b, sr_b):
    ...
    # Add genre/feel expression based on MIR profiling
    if genre in ['jazz', 'blues', 'hiphop', 'reggae']:
    ...
    # Adaptive Velocity Thresholding (Median-based MIR approach)
    if len(raw_notes) > 0:
        avg_vel = np.median([ev.velocity for ev in raw_notes])
        vel_threshold = max(20, avg_vel * 0.45) # Dynamic noise floor
        ghost_threshold = max(30, avg_vel * 0.70) # Dynamic ghost note threshold
    else:
        vel_threshold = 45
        ghost_threshold = 50
    
    for ev in raw_notes:
        # 15 = D#0 (rare but possible with octavers), 72 = C5 (high C on 6-string bass)
        if ev.pitch < 15 or ev.pitch > 72: continue
        
        if level == "simplex":
            beat_loc = ev.start * (bpm / 60.0)
            if abs(beat_loc % 1) > 0.35: # Keep quarter note pulse, not just half notes
                continue
        elif level == "normal":
    ...
    # Extract and insert chords for harmonic context
    ...
    if level == "simplex":
        ...
        if genre in ['jazz', 'blues', 'reggae']:
            factor = 3 # 8th note triplets
        else:
            factor = 4 # 16th notes
        min_len = 0.25
    else: # complex
        if genre in ['jazz', 'blues', 'reggae']:
            factor = 6 # 16th note triplets
    ...
    for i in range(len(quantized_events) - 1):
        ...
        # Legato Smoothing: Eliminate excessive small rests to produce cleaner notation
        # Bass lines are often played legato; a tiny gap is usually just fingering transition.
        ...
        if 0 < gap <= 0.5: # Close gaps up to an 8th note
            quantized_events[i]["end"] = quantized_events[i+1]["start"]
            # If the original note was short (<= 16th) and we extended it significantly, mark it staccato
    ...
    for idx, q_ev in enumerate(quantized_events):
        ...
        # Identify Grace Notes (very short transitional notes)
        is_grace = False
    ...
    # Identify and add slurs for legato (hammer-on, pull-off) and glissando (slides)
    for idx, q_ev in enumerate(quantized_events):
        el1 = q_ev.get("music_element")
        if not el1: continue
        
        # Check for pitch bend inside this note to detect string bends/slides and vibrato
        ...
        if bends_in_note:
            ...
            # If there's a significant oscillation in pitch bends during a long note
            if (q_ev["end"] - q_ev["start"]) > 0.4 and (max_bend - min_bend) > 200:
                # Count zero crossings in the derivative of the pitch bend
        ...
        if has_vibrato:
            el1.articulations.append(articulations.Doit()) # Using Doit as a placeholder visual for vibrato bend if true vibrato not natively supported in standard MusicXML easily, though expressions.Trill or standard text works
            el1.addLyric("vib.")
    ...
    if len(part.flatten().notesAndRests) > 0:
        ...
        part.makeNotation(meter=ts, inPlace=True) # Improves rhythmic spelling (syncopations/ties across beats)
        sc.append(part)
...
def main():
    ...
    # Automatically derive the genre profile, tempo, and baseline tracks based on spectral logic
    if genre_override != "auto":
        print(f"Using manual genre override: [{genre_override.upper()}]")
